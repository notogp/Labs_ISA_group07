...
const int b[NT]={-13, -28, 104, 544, 830, 544, 104, -28, -13}; /// b array
...
int myfilter(int x)
{
  ...
  //This cicle will create the final value of y and then it will shift (divide for 2^11) it 
  //in order to convert them in integer.

  for (i=0; i<NT; i++){
    y += (sx[i]*b[i]) >> (NB-1);
    y_not_shifted += (sx[i]*b[i]);
  }

  printf("%d ", y_not_shifted);
  y_shifted=y_not_shifted >> 11;
  printf("%d ", y_shifted);
